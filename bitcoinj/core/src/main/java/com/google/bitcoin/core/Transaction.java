...
public class Transaction extends ChildMessage implements Serializable {
    ...
    public synchronized byte[] computeScriptBytes(final int inputIndex, final SigHash hashType, final Wallet wallet) throws ScriptException {
        ...
        final TransactionInput input = inputs.get(inputIndex);
        
        final ECKey key = input.getOutpoint().getConnectedKey(wallet);
        ...
        final byte[] connectedPubKeyScript = input.getOutpoint().getConnectedPubKeyScript();
        
        final Sha256Hash hash = this.hashTransactionForSignature(inputIndex, connectedPubKeyScript, hashType, anyoneCanPay);
        
        byte[] signature = null;
        
        try {
        	
            final ByteArrayOutputStream bos = new UnsafeByteArrayOutputStream(73);
        ...
        final Script scriptPubKey = input.getOutpoint().getConnectedOutput().getScriptPubKey();
        
        if (scriptPubKey.isSentToAddress()) {
        	
            return Script.createInputScript(signature, key.getPubKey());
        } 
        else if (scriptPubKey.isSentToRawPubKey()) {
        	
            return Script.createInputScript(signature);
        } 
        else {
